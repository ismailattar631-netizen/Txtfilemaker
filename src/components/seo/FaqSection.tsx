        ...
        <div className="space-y-3">
          {faqs.map((faq, idx) => {
            ...
            return (
              <div
                key={idx}
                className="border border-slate-800 rounded-xl bg-slate-900/60 overflow-hidden transition-all duration-200"
              >
                <button
                  onClick={() => toggle(idx)}
                  className="w-full flex items-center justify-between p-5 text-left font-medium text-slate-200 hover:text-teal-400 transition-colors"
                ...
                {isOpen && (
                  <div className="px-5 pb-5 text-sm sm:text-base text-slate-400 leading-relaxed border-t border-slate-800/40 pt-3">
                    {faq.answer}
                  </div>
                )}
              </div>
