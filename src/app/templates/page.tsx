    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10 transition-colors">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
            ...
            <button
              ...
              className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition-all ${
                activeCategory === cat.id
                  ...
                  : 'bg-white text-slate-600 hover:text-slate-900 border border-slate-200 hover:border-slate-300 dark:bg-slate-900/80 dark:text-slate-400 dark:hover:text-slate-200 dark:border-slate-800 dark:hover:border-slate-700 shadow-sm'
              }`}
            >
              {cat.label}
            </button>
        ...
        <div className="relative w-full sm:w-64">
          ...
          <input
            ...
            className="w-full bg-white border border-slate-200 dark:bg-slate-900 dark:border-slate-800 rounded-xl pl-9 pr-4 py-2 text-xs text-slate-800 dark:text-slate-200 placeholder-slate-400 dark:placeholder-slate-500 focus:outline-none focus:border-teal-500 shadow-sm"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filteredTemplates.map((template) => (
          <div
            key={template.slug}
            className="group rounded-2xl bg-white border border-slate-200 shadow-sm hover:border-teal-500 dark:bg-slate-900/60 dark:border-slate-800 dark:hover:border-teal-500/40 p-5 flex flex-col justify-between transition-all hover:shadow-xl hover:shadow-teal-500/5 space-y-4"
          >
            <div className="space-y-2.5">
              <div className="flex items-center justify-between">
                <span className="text-[11px] font-mono px-2 py-0.5 rounded bg-slate-100 dark:bg-slate-950 text-teal-700 dark:text-teal-400 border border-slate-200 dark:border-slate-800 font-semibold">
              ...
              <Link href={`/templates/${template.slug}`}>
                <h3 className="font-bold text-slate-900 dark:text-slate-100 group-hover:text-teal-600 dark:group-hover:text-teal-400 transition-colors text-base line-clamp-1">
                  {template.title}
                </h3>
              </Link>
              ...
              <div className="bg-slate-50 dark:bg-slate-950 p-2.5 rounded-xl border border-slate-200 dark:border-slate-850 font-mono text-[11px] text-slate-600 dark:text-slate-400 line-clamp-3 overflow-hidden select-none">
                {template.content}
              </div>
            </div>
            ...
            <div className="flex items-center justify-between pt-2 border-t border-slate-100 dark:border-slate-800/80">
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-xl bg-slate-100 hover:bg-slate-200 text-slate-700 dark:bg-slate-800 dark:hover:bg-slate-700 dark:text-slate-200 text-xs font-medium flex items-center gap-1.5 transition-colors shadow-sm"
