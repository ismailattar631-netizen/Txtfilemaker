    ...
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-10 transition-colors">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950 overflow-hidden shadow-xl">
          <div className="bg-slate-100 dark:bg-slate-900 px-4 py-2.5 border-b border-slate-200 dark:border-slate-800 text-xs font-bold text-purple-600 dark:text-purple-400 uppercase tracking-wider">
            Input Markdown / HTML
          </div>
          <textarea
            value={input}
            ...
            rows={14}
            className="w-full bg-white text-slate-900 placeholder-slate-400 dark:bg-slate-950 dark:text-slate-100 dark:placeholder-slate-600 p-4 font-mono text-xs sm:text-sm outline-none resize-none custom-scrollbar leading-relaxed"
          />
        </div>

        {/* Output */}
        <div className="rounded-2xl border border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950 overflow-hidden shadow-xl flex flex-col">
          <div className="bg-slate-100 dark:bg-slate-900 px-4 py-2.5 border-b border-slate-200 dark:border-slate-800 flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button
                onClick={handleCopy}
                className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-white border border-slate-300 hover:bg-slate-50 dark:bg-slate-800 dark:border-slate-700 dark:hover:bg-slate-700 text-xs font-medium text-slate-700 dark:text-slate-200 shadow-sm"
              >
              ...
              <button
                ...
                className="flex items-center gap-1.5 px-3 py-1 rounded-lg bg-purple-500 hover:bg-purple-400 text-slate-950 text-xs font-bold shadow-sm"
              >
          ...
          <textarea
            ...
            value={cleanText}
            rows={14}
            className="w-full flex-1 bg-white text-slate-900 dark:bg-slate-950 dark:text-slate-100 p-4 font-mono text-xs sm:text-sm outline-none resize-none custom-scrollbar leading-relaxed"
          />
        </div>
      </div>
    </div>
